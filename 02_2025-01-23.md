...
### Kapcsolódó szavak, kifejezések, rövidítések
...
- Férfi: Elnézést, meg tudná mondani nekem, mennyi az idő?
- Nő: Igen, természetesen! Fél három van.
- Férfi: Ön nagyon csinos! Megkaphatom a telefonszámát?
- Nő: Elnézést, nem értek izlandiul!

> [!IMPORTANT]
> Figyeljük meg, hogy az "_elnézést meg tudná mondani, hogy mennyi az idő_" mondatban a `hvað klukkan er` egyenes szórendben van, hiszen nem azt kérdezzük, mennyi az idő, hanem azt, hogy meg tudná-e mondani!

> [!IMPORTANT]
> Az `að geta` (ragozása `ég get`, `þú getur`, `hann getur`) segédige az ige 3. alakját vonzza. Azaz pl. `baka` → `bakað` (_megsült_/_megsülve_), `borga` → `borgað` (_kifizetve_/_kifizetett_), de `segja` → `sagt` (_mondva_/_mondott_)
> Azaz a `geturðu sagt` (`getur þú sagt`) szó szerint _tudsz te mondva_, értelem szerint _meg tudod mondani_
> A `getur` a _tudás_t a "can" (_lehetőség_) értelemben érti, ellentétben a `kann`-nal, ami meg a "to be able to" (fizikai képesség)
> A `getur` sokkal gyakoribb értelemszerűen a beszélt nyelvben, mint a `kann`
> A `getur` jelentheti azt, amit a `kann`, de fordítva nem

> [!IMPORTANT]
> `Má ég fá ...?` jelentése "_megkaphatom ...?_", tárgyesetet vonz, mivel `símanúmer(ið)` `hvk.` (_a telefonszám_), ezért itt a tárgyeset és az alanyeset azonos
> `símanúmerið þitt` jelentése "_a telefonszámod(at)_", itt azért `þitt` van, mert semlegesnemű a főnév. (`þinn`: `kk.`, `þín`: `kvk.`, `þitt`: `hvk.`)
...
|------------------|-----------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `að gefa`        | _adni_                            |                                                                                                                                                                                                                                       |
| `að fyrirgefa`   | _megbocsátani_                    | Azaz angolul ez szó szerint _for give_ = `fyrir gefa`, _forgive_ = _megbocsátani_                                                                                                                                                     |
| `að afsaka`      | _bocsánatot kérni_                | -                                                                                                                                                                                                                                     |
| `fyrirgefðu`     | _elnézést_ (hibázásért)           | Ez az egyes számú (második személyű), felszólító módú formája az `að fyrirgefa`-nak (úgy mint `fyrirgef þú`). Ez inkább jelenti a "sorry" értelmét az _elnézést_nek, tehát pl. ha véletlen a lábára lépsz valakinek, akkor mondod ezt |
| `afsakið`        | _elnézést_ (figyelemfelhívásként) | Ez a többes számú, felszólító módú formája az `að afsaka`-nak ("_bocsássanak meg (nekem)_"). Ez az angol "excuse me" értelemben vett bocsánatkérést jelenti, tehát nem tényleges bocsánatot kérsz, hanem figyelmet kérsz              |
| `að geta`        | _tudni_                           | (lásd important írást)                                                                                                                                                                                                                |
| `mér`            | _nekem_                           | -                                                                                                                                                                                                                                     |
| `alveg`          | _teljesen_                        | -                                                                                                                                                                                                                                     |
| `sjálfsagt`      | _természetesen_                   | szó szerint: _magát mondja_ (kb. magától értetődik)                                                                                                                                                                                   |
| `fallegur` `kk.` | _szép_/_csinos_                   | -                                                                                                                                                                                                                                     |
| `falleg` `kvk.`  | _szép_/_csinos_                   | -                                                                                                                                                                                                                                     |
| `fallegt` `hvk.` | _szép_/_csinos_                   | -                                                                                                                                                                                                                                     |
| `má`             | _-hat/-het_                       | Mint az angol "may"                                                                                                                                                                                                                   |
